 ...
 */
export async function run (install = true, _target: FoundryupTarget = {}): Promise<boolean> {
  if (install) {
  ...
  try {
    ...
    if (!status) {
      console.error(
        ...
        'Consider installing via `curl -L https://foundry.paradigm.xyz | bash` ' +
        'and then running `foundryup` on a new terminal. ' +
        'For more context, check the installation instructions ' +
        'in the book: https://book.getfoundry.sh/getting-started/installation.html.',
      );
    }
