...
abstract contract BlocklistStorage is IBlocklistStorage {
    ...
    function isBlocked(uint256 tokenId) public view override returns (bool) {
        return
            !_isBlocklistPaused() &&
            StorageSlotUpgradeable.getBooleanSlot(keccak256(abi.encodePacked(_BLOCKLIST_PREFIX_SLOT, tokenId))).value;
    }

    function areBlocked(uint256[] calldata tokenIds) public view override returns (bool[] memory values) {
