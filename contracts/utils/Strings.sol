...
library Strings {
    ...
    function contains(slice memory self, slice memory value) internal pure returns (bool) {
        return rfindPtr(self._len, self._ptr, value._len, value._ptr) != self._ptr;
    }

    // Returns the memory address of the first byte after the last occurrence of
    // `value` in `self`, or the address of `self` if not found.
    function rfindPtr(uint selflen, uint selfptr, uint valuelen, uint valueptr) private pure returns (uint) {
        uint ptr;
