...
describe('UNSRegistry (proxy)', () => {
  ...
  describe('Registry', () => {
    ...
    it('should resolve properly', async () => {
      const tokenId = await mintDomain(unsRegistry, owner.address, TLD.CRYPTO, 'resolution');
      await unsRegistry.burn(tokenId);

      await unsRegistry['mint(address,uint256,string)'](owner.address, tokenId, 'resolution');
  ...
  describe('Resolver', () => {
    ...
    it('should get key by hash', async () => {
      const tokenId = await mintDomain(unsRegistry, owner.address, TLD.CRYPTO, 'heyhash');
    ...
    it('should get many keys by hashes', async () => {
      const tokenId = await mintDomain(unsRegistry, owner.address, TLD.CRYPTO, 'heyhash-many');
    ...
    it('should not consume additional gas if key hash was set before', async () => {
      const tokenId = await mintDomain(unsRegistry, owner.address, TLD.CRYPTO, 'heyhash-gas');
    ...
    it('should get value by key hash', async () => {
      const tokenId = await mintDomain(unsRegistry, owner.address, TLD.CRYPTO, 'get-key-by-hash');
    ...
    it('should get multiple values by hashes', async () => {
      const tokenId = await mintDomain(unsRegistry, owner.address, TLD.CRYPTO, 'get-many-keys-by-hash');
    ...
    it('should emit NewKey event new keys added', async () => {
      const tokenId = await mintDomain(unsRegistry, owner.address, TLD.CRYPTO, 'new-key');
      const key = 'new-key';
      const value = 'value';
    ...
    it('should emit correct Set event', async () => {
      const tokenId = await mintDomain(unsRegistry, owner.address, TLD.CRYPTO, 'check-set-event');
      const key = 'new-key';
      const value = 'value';
    ...
    it('should reconfigure resolver with new values', async () => {
      const tokenId = await mintDomain(unsRegistry, owner.address, TLD.CRYPTO, 'reconfigure');
