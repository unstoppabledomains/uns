...
describe.skip('Resolver', () => {
  let Registry, MintingController, Resolver;
  let mintingController, registry, resolver;
  let signers, notOwnerSigner;
  ...
  it('should resolve tokens', async () => {
    ...
    await expect(resolver.set('key', 'value', tok))
    ...
    await expect(resolver.set('key', 'value', tok))
      .to.be.revertedWith('SENDER_IS_NOT_APPROVED_OR_OWNER');

    await expect(resolver.get('key', tok))
      .to.be.revertedWith('');
  })
