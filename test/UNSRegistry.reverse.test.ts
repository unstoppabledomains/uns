...
describe('UNSRegistry (reverse)', () => {
  ...
  let signers: SignerWithAddress[], coinbase: SignerWithAddress, owner: SignerWithAddress, receiver: SignerWithAddress, reader: SignerWithAddress;
  ...
  describe('Minting', () => {
    ...
    describe('mintWithRecords(address,string[],string[],string[])', async () => {
      ...
      it('should produce SetReverse event after NewURI', async () => {
        ...
        expect(mintTxReceipt.events?.[setReverseIndex!].args).to.deep.equal([ // eslint-disable-line @typescript-eslint/no-non-null-assertion
          coinbase.address,
          tokenId,
        ]);
