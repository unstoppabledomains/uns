...
function expectEvent(receipt, eventName, eventArgs = {}) {
  ...
  if (isWeb3Receipt(receipt)) {
    const logs = Object.keys(receipt.events)
      .map(name => {
        if (Array.isArray(receipt.events[name])) {
          return receipt.events[name].map(event => ({
            event: name,
...
function notInLogs(logs, eventName) {
  ...
  expect(logs.find(e => e.event === eventName), `Event ${eventName} was found`)
    .to.be.undefined
}
...
async function inConstruction(contract, eventName, eventArgs = {}) {
  if (!isTruffleContract(contract)) {
    throw new Error(
      'expectEvent.inConstruction is only supported for truffle-contract objects',
    )
  }
...
function contains(args, key, value) {
  ...
  if (value === null) {
    ...
    expect(args[key]).to.be.bignumber.equal(
      value,
