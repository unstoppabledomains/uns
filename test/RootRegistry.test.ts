...
describe('RootRegistry', () => {
  ...
  before(async () => {
    [registryOwner, rcmOwner, predicateOwner, owner, spender] =
      await ethers.getSigners();

    l1UnsRegistry = await new UNSRegistry__factory(registryOwner)
    ...
    checkpointManager = await new SimpleCheckpointManager__factory(
      rcmOwner,
    ).deploy();

    // deploy and init predicate
    predicate = await new MintableERC721Predicate__factory(predicateOwner)
      .connect(predicateOwner)
      .deploy();
    await predicate.initialize(predicateOwner.address);

    // deploy and setup root chain manager
    rootChainManager = await new RootChainManager__factory(rcmOwner)
      .connect(rcmOwner)
      .deploy();
    ...
    buildExecuteUnsParams = buildExecuteFunc(
      l1UnsRegistry.interface,
      l1UnsRegistry.address,
      l1UnsRegistry,
    );
  });
  ...
  describe('Deposit', () => {
    ...
    describe('Two-steps deposit', () => {
      it('should deposit token', async () => {
        ...
        await expect(
          rootChainManager
            .connect(owner)
            .depositFor(owner.address, l1UnsRegistry.address, data),
        )
          .to.emit(predicate, 'LockedMintableERC721')
          .withArgs(
            owner.address,
    ...
    describe('CNS -> UNS migration', () => {
      ...
      it('should meta-migrate CNS domain to UNS through safeTransferFrom', async () => {
        ...
        await signatureController
          .connect(spender)
          [funcSig](owner.address, l1UnsRegistry.address, tokenId, signature);
      ...
      it('should revert when UNS registry receives token from random ERC721', async () => {
        const randomERC721 = await new CNSRegistry__factory(
          registryOwner,
        ).deploy();
        await randomERC721.controlledMintChild(
          owner.address,
          TLD.CRYPTO,
          'fake-cns-uns-te1',
        );
        const tokenId = await randomERC721.childIdOf(
          TLD.CRYPTO,
          'fake-cns-uns-te1',
        );
  ...
  describe('Withdraw through predicate', () => {
    it('should withdraw a domain', async () => {
      ...
      const inputData = buildPredicateExitInput(
        owner.address,
        ZERO_ADDRESS,
        tokenId,
      );
  ...
  describe('Withdraw', () => {
    const expectNewHeaderBlockEventEmitted = async (
      setCheckPointTx,
      checkpointData,
    ) => {
    ...
    it('should be able to exit through rootChainManager', async () => {
      ...
      const data = await buildExitInput(
        checkpointManager,
        receipt,
        checkpointData,
      );
    ...
    it('should be able to exit through UNS registry', async () => {
      ...
      await l1UnsRegistry
        .connect(owner)
        .withdrawFromPolygon(data, tokenId, [], []);
    ...
    it('should be able to exit through UNS registry with records update', async () => {
      ...
      await l1UnsRegistry
        .connect(owner)
        .withdrawFromPolygon(data, tokenId, ['k1'], ['v1']);
