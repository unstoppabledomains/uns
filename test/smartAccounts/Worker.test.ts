...
describe('Worker Smart Account', () => {
  ...
  let faucetImplementation: Faucet;
  let faucet: Faucet;
  let workerSAImplementation: Worker;
  ...
  before(async () => {
    ...
    faucetImplementation = await faucetFactory.deploy();

    faucetWallet = new ethers.Wallet(Wallet.createRandom().privateKey, ethers.provider);

    const faucetAuth: Authorization = await faucetWallet.authorize({ address: faucetImplementation.target, nonce: 1 });
  ...
  describe('basic tests', async () => {
    ...
    beforeEach(async () => {
      ...
      workerAsContract = await ethers.getContractAt('Worker', workerWallet.address);
    });
    ...
    it('should fail and correctly pass contract error class from internal call', async () => {
      ...
      const faucetWithdrawData = localFaucet.interface.encodeFunctionData('requestFunding');
