...
describe('BaseForwarder', () => {
  ...
  let ERC1271SimpleWalletFactory, simpleWallet;
  let ERC1271MockFactory, mockWallet;
  ...
  before(async () => {
    ...
    ERC1271SimpleWalletFactory = await ethers.getContractFactory(
      'ERC1271SimpleWallet',
    );
    ERC1271MockFactory = await ethers.getContractFactory('ERC1271Mock');

    forwarder = await BaseForwarder.deploy();
    simpleWallet = await ERC1271SimpleWalletFactory.deploy(owner.address);
