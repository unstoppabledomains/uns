  ...
  describe('Registry (minting)', () => {
    describe('mintWithRecords(address,uint256,string,string[],string[])', async () => {
      ...
      it('should produce NewURI event', async () => {
        ...
        await expect(unsRegistry.mintWithRecords(coinbase.address, labels, ['key1'], [42]))
      ...
      it('should not allow minting subdomain if not minting manager', async () => {
        ...
        labels.unshift('sub');
        const _registry = await unsRegistry.connect(owner);
  ...
  describe('Registry (records management)', () => {
    ...
    describe('reset', () => {
      it('should reset all records', async () => {
        ...
        expect(await unsRegistry.getMany(['key1', 'key2'], tokenId)).to.be.deep.eql(['', '']);
      });
      ...
      it('should fail if not owner', async () => {
        ...
        expect(await unsRegistry.getMany(['key1', 'key2'], tokenId)).to.be.deep.eql(['value1', 'value2']);
      });
    });
